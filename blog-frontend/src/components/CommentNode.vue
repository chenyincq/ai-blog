...
<template>
  <li class="comment-item" :class="{ 'comment-reply': node.parentId }">
    <div class="comment-meta">{{ node.author }} · {{ node.createdAt?.slice(0, 16)?.replace('T', ' ') }}</div>
    ...
    <div v-if="replyingTo === node.id" class="comment-reply-form">
      <input :value="replyAuthor" @input="$emit('update:replyAuthor', $event.target.value)" type="text" placeholder="昵称" maxlength="50" />
      <textarea :value="replyContent" @input="$emit('update:replyContent', $event.target.value)" placeholder="回复…" rows="2"></textarea>
